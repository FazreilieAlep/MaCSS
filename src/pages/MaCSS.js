...
function MaCSS() {
    ...
    const changeCursor = useCallback(() => {
        const cursorLogic = () => {
            ...
            if (lastYPos >= quoteBorder && maskSize > 0) {
                const timeout = setTimeout(() => {
                    setMaskSize(0);
                }, 200);

                return () => clearTimeout(timeout);
            } else if (lastYPos < quoteBorder && maskSize === 0) {
                const timeout = setTimeout(() => {
                    initialMaskSize();
                }, 200);
        ...
        if (lastYPos < headerBorder || (lastYPos >= aboutSiteBorder && lastYPos < projectBorder) ) {
            if (isMask) {
                const timeout = setTimeout(() => {
                    setIsMask(false);
                    setMaskOpacity(0);
    ...
    }, [lastYPos, headerBorder, aboutMeBorder, aboutSiteBorder, isMask, maskImage, projectBorder, quoteBorder, maskSize]);
    ...
    return (
        <div ref={pageRef} className='MaCSS-page'>
            <motion.div
                    ...
                    transition={{ duration: tweenBackOut }}
                />
            <Navbar></Navbar>
            {/* <Navbar2></Navbar2> */}
            {/* Main Layer */}
